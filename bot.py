...
def calculate_best_bet(player_score, banker_score):
    ...
    if banker_prob > player_prob:
        previous_suggestion = "莊" if random.random() > 0.5 else "閒"
    else:
        previous_suggestion = "閒" if random.random() > 0.5 else "莊"
    
    next_bet_amount = round((base_bet * (1.5 if total_wins >= 2 else 1.25 if total_losses >= 3 else 1)) / 50) * 50  
...
def handle_message(event):
    ...
    if user_input == "開始":
        ...
        return line_bot_api.reply_message(event.reply_token, TextSendMessage(text=f"💵 本次遊戲結束，剩餘資金：${balance}"))
        ...
        try:
            ...
            return line_bot_api.reply_message(event.reply_token, TextSendMessage(text="輸入格式錯誤，請重新輸入，如 '8 9'"))
